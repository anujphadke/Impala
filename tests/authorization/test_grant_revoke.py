...
class TestGrantRevoke(CustomClusterTestSuite, ImpalaTestSuite):
  ...
  def test_role_privilege_case(self, vector):
    ...
    """
    try:
      db_name = "grant_rev_db_" + get_random_id(5)
      db_name_upper_case = "XDB_" + get_random_id(5)
      db_name_mixed_case = "YdB_" + get_random_id(5)
      role_name = "test_role_" + get_random_id(5)
      self.client.execute("create role {0}".
          format(role_name))
      self.client.execute("grant all on server to {0}".
          format(role_name))
      self.client.execute("grant role {0} to group {1}".
          format(role_name, grp.getgrnam(getuser()).gr_name))
      ...
      self.client.execute("create table if not exists {0}.test1(i int)".
          format(db_name))
      self.client.execute("create table if not exists {0}.TEST2(i int)".
          format(db_name))
      self.client.execute("create table if not exists {0}.Test3(i int)".
          format(db_name))

      self.client.execute("grant select on table test1 to {0}".
          format(role_name))
      self.client.execute("grant select on table TEST2 to {0}".
          format(role_name))
      self.client.execute("grant select on table TesT3 to {0}".
         format(role_name))
      self.client.execute("grant all on database {0} to {1}".
          format(db_name, role_name))
      self.client.execute("grant all on database {0} to {1}".
          format(db_name_upper_case, role_name))
      self.client.execute("grant all on database {0} to {1}".
          format(db_name_mixed_case, role_name))
      result = self.client.execute("show grant role {0}".
          format(role_name))
      ...
      self.client.execute("drop database if exists {0}".
          format(db_name_upper_case))
      self.client.execute("drop database if exists {0}".
          format(db_name_mixed_case))
      self.client.execute("drop database if exists {0} cascade".
          format(db_name))
      self.client.execute("drop role {0}".
          format(role_name))
