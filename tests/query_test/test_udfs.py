...
class TestUdfTargeted(TestUdfBase):
  ...
  def test_concurrent_jar_drop_use(self, vector, unique_database):
    ...
    create_fn_to_drop = "create function {0}.foo_{1}() returns string LOCATION '{2}' SYMBOL='org.apache.impala.TestUpdateUdf'"
    create_fn_to_use = "create function {0}.use_it(string) returns string LOCATION '{1}' SYMBOL='org.apache.impala.TestUdf'"
    drop_fn = "drop function if exists {0}.foo_{1}()"
    use_fn = "select * from (select max(int_col) from functional.alltypesagg where {0}.use_it(string_col) = 'blah' union all (select max(int_col) from functional.alltypesagg where {0}.use_it(String_col) > '1' union all (select max(int_col) from functional.alltypesagg where {0}.use_it(string_col) > '1'))) v"
    num_drops = 100
    num_uses = 100
