...
//
// The results of the above subtractions are stored in Decimal16Value to avoid overflowing
...
BigIntVal MathFunctions::WidthBucketImpl(FunctionContext* ctx,
  ...
  Decimal16Value dist_from_min;
  dist_from_min = expr.template Subtract<int128_t>(input_scale, min_range, input_scale,
  ...
  if (int total_leading_zeros = BitUtil::CountLeadingZeros(abs(buckets.value())) +
      BitUtil::CountLeadingZeros(abs(dist_from_min.value())) > 128) {
    if (abs(buckets.value()) > DecimalUtil::MAX_UNSCALED_DECIMAL16 /
        abs(dist_from_min.value())) {
      needs_int256 = true;
    }
  }
  ...
  if (!needs_int256) {
    // Statically check if the reult of multiplication would not fit in 128 bits
    if(detail::MaxBitsRequiredAfterScaling(range_size.value(), input_scale) > 128) {
      // Use the leading zero count test to check if the multiplication would overflow
      int total_leading_zeros = BitUtil::CountLeadingZeros(range_size.value()) +
          BitUtil::CountLeadingZeros(DecimalUtil::GetScaleMultiplier<int128_t>(
          input_scale));
      if (total_leading_zeros <= 128) {
        if (abs(range_size.value()) > DecimalUtil::MAX_UNSCALED_DECIMAL16 /
            DecimalUtil::GetScaleMultiplier<int128_t>(input_scale)) {
          needs_int256 = true;
        }
      }
    }
